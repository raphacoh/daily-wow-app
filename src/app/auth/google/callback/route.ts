...
export async function GET(req: NextRequest) {
  ...
  const fail = (why: string) => {
    ...
    const r = NextResponse.redirect(new URL("/signin?error=google", url.origin));
    for (const c of ["g_state", "g_nonce", "g_next"]) r.cookies.set(c, "", { path: "/auth/google", maxAge: 0 });
    return r;
  };
  if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_CLIENT_SECRET || !supabaseConfigured()) return fail("not configured");
  ...
  if (!code || !state || !savedState || state !== savedState || !nonce) return fail("state mismatch");
  ...
  if (!tokens.id_token) return fail("no id_token");
