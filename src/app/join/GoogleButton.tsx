...
export function GoogleButton({ next, label = "להמשיך עם Google" }: { next: string; label?: string }) {
  ...
  // A link, not a form: browsers apply CSP form-action to a form's redirect chain (which ends at Google), links are free.
  if (direct()) {
    return (
      <div className="gbtn-wrap">
        <a className="btn ghost gbtn" href={`/auth/google?next=${encodeURIComponent(next)}`}>
          {icon}
          <span>{label}</span>
        </a>
