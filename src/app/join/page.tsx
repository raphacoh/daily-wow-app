...
import { t } from "@/i18n";
import { currentParent, googleSignInEnabled } from "@/lib/auth";
...
export default async function JoinPage() {
  ...
  return (
    <main className="page">
      <JoinForm levels={levels} grades={[...GRADES]} gradeByAge={gradeByAge} ages={AGES} maxKids={MAX_KIDS} presetEmail={parent?.email ?? ""} presetName={parent?.name ?? ""} google={googleSignInEnabled() && !parent ? <GoogleButton next="/join" /> : null} />
    </main>
  );
}
