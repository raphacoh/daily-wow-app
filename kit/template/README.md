...
The engine emits one event at each decision point through `WOW.emit(type, payload)` — quick-checks, MCQs, the ordering task, the numeric tasks, the challenge, the explanation grade, the predict-first control, the bonus/sources toggles, and `finish`. Outside the app `window.WOW` does not exist and the shim makes every call a no-op; inside the app (`public/wow-runtime.js`) they become the kid's wings, medals, cards and badges (see `docs/gamification-spec.md`). `ENGINE_VERSION` says which hooks a fragment carries. Keep the shim and the calls when you build a new edition; keep the item ids structural.
